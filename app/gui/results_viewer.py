...
class ResultsViewWidget(QWidget):
    ...
    def _setup_table_tab(self) -> None:
        ...
        for m in sorted(list(unique_methods)):
            self.method_combo.addItem(self._display_method_name(m), m)
    ...
    def _populate_table(self, *args) -> None:
        ...
        for key, df in self.display_results.items():
            ...
            for _, row in top_5.iterrows():
                ...
                is_comp = bool(row.get("Is_Compromise", False))
                ...
                display_id = comp_id + (" ⭐ (Compromise)" if is_comp else "")

                summary_rows.append({
                    "Method": self._display_method_name(method),
                    "Scheme": scheme,
                    "Rank": rank,
                    "Company ID": display_id,
                    "Score": score,
                })
    ...
    def _setup_export_tab(self) -> None:
        ...
        path_lbl.setStyleSheet("font-size: 22pt; color: #555555; background-color: #eef2f3; padding: 12px; border-radius: 6px;")
