...
import numpy as np
from app.core.pairwise_math import calculate_ahp_weights, calculate_fahp_weights
...
class WeightSchemeDialog(QDialog):
    ...
    def _abbrev(name: str) -> str:
        """Return the initials of each capitalized word, e.g. 'Quality Performance' -> 'QP'."""
    ...
    def __init__(self, entity_type: str, parent: QWidget | None = None) -> None:
        ...
        self.setMinimumSize(DIMENSIONS["dialog_width"] + 100, DIMENSIONS["dialog_height"])
        self.resize(1800, DIMENSIONS["dialog_height"])
    ...
    def _setup_create_tab(self) -> None:
        ...
        tab_layout.addWidget(right_pane, 1)

        self.tab_widget.addTab(tab, "Create Direct Scheme")
        self.update_preview()

    def _setup_ahp_tab(self) -> None:
        """Set up the 'Create AHP Scheme' tab layout."""
        tab = QWidget()
        tab_layout = QHBoxLayout(tab)
        tab_layout.setContentsMargins(14, 14, 14, 14)
        tab_layout.setSpacing(14)
        ...
        name_layout = QHBoxLayout()
        name_lbl = QLabel("Scheme Name:")
        self.ahp_name_input = QLineEdit()
        self.ahp_name_input.setPlaceholderText("e.g. AHP Pairwise Matrix...")
        name_layout.addWidget(name_lbl)
        name_layout.addWidget(self.ahp_name_input)
        left_layout.addLayout(name_layout)

        n = len(self.criteria_names)
        self.ahp_table = QTableWidget(n, n)
        abbrev_names = [self._abbrev(n) for n in self.criteria_names]
        self.ahp_table.setHorizontalHeaderLabels(abbrev_names)
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i == j:
                    ...
                    row_combos.append(None)
                elif i < j:
                    combo = QComboBox()
                    # Values from 1/9 to 9
                    vals = ["1/9", "1/8", "1/7", "1/6", "1/5", "1/4", "1/3", "1/2", 
                            "1", "2", "3", "4", "5", "6", "7", "8", "9"]
                    combo.addItems(vals)
                    combo.setCurrentText("1")
                    combo.setProperty("row", i)
                    combo.setProperty("col", j)
                    combo.currentTextChanged.connect(self._on_ahp_matrix_changed)
                    self.ahp_table.setCellWidget(i, j, combo)
                    ...
                    self.ahp_table.setItem(i, j, item)
                    row_combos.append(None)
            self.combo_matrix.append(row_combos)

        # resize headers
        header = self.ahp_table.horizontalHeader()
        header.setMinimumSectionSize(120)
        header.setSectionResizeMode(header.__class__.ResizeMode.ResizeToContents)
        
        v_header = self.ahp_table.verticalHeader()
        v_header.setDefaultSectionSize(50)
        v_header.setMinimumSectionSize(50)
        
        left_layout.addWidget(self.ahp_table)

        # fuzzy option
        self.use_fahp_chk = QCheckBox("Use Fuzzy AHP (Triangular membership)")
        left_layout.addWidget(self.use_fahp_chk)

        # buttons
        btn_layout = QHBoxLayout()
        self.ahp_calc_btn = QPushButton("Calculate & Preview")
        self.ahp_calc_btn.clicked.connect(self._preview_ahp)
        self.ahp_save_btn = QPushButton("Save AHP Scheme")
        self.ahp_save_btn.setProperty("class", "success")
        self.ahp_save_btn.style().unpolish(self.ahp_save_btn)
        self.ahp_save_btn.style().polish(self.ahp_save_btn)
        self.ahp_save_btn.clicked.connect(self.save_ahp_scheme)
        self.ahp_save_btn.setEnabled(False)
        btn_layout.addWidget(self.ahp_calc_btn)
        btn_layout.addWidget(self.ahp_save_btn)
        ...
        preview_title = QLabel("AHP Results Preview")
        preview_title.setProperty("role", "subheader")
        right_layout.addWidget(preview_title)

        self.ahp_preview_text = QTextBrowser()
        self.ahp_preview_text.setFrameShape(QFrame.Shape.NoFrame)
        right_layout.addWidget(self.ahp_preview_text)
        tab_layout.addWidget(right_pane, 1)

        self.tab_widget.addTab(tab, "Create AHP Scheme")

    def _on_ahp_matrix_changed(self, text: str) -> None:
        combo = self.sender()
        if not combo: return
        ...
        if "/" in text:
            try:
                val = float(text.split("/")[0]) / float(text.split("/")[1])
            except:
                pass
        else:
            try:
        ...
        recip_text = ""
        if val > 1.0:
            recip_text = f"1/{int(val)}"
        elif val < 1.0:
            recip_text = str(int(1.0/val))
        else:
            recip_text = "1"
            
        item = self.ahp_table.item(c, r)
        if item:
            item.setText(recip_text)
            
        self.ahp_save_btn.setEnabled(False) # need to recalc

    def _get_ahp_matrix(self) -> np.ndarray:
        n = len(self.criteria_names)
        matrix = np.zeros((n, n))
        for i in range(n):
            for j in range(n):
                if i == j:
                    ...
                    combo = self.combo_matrix[i][j]
                    text = combo.currentText()
                    if "/" in text:
                        try:
                            matrix[i, j] = float(text.split("/")[0]) / float(text.split("/")[1])
                        except:
                            matrix[i, j] = 1.0
                    else:
                        try:
    ...
    def _preview_ahp(self) -> None:
        matrix = self._get_ahp_matrix()
        
        if self.use_fahp_chk.isChecked():
            weights = calculate_fahp_weights(matrix, self.criteria_names)
            _, standard_cr = calculate_ahp_weights(matrix, self.criteria_names)
            html_lines = ["<p><b>Fuzzy AHP Weights:</b></p><ul>"]
            for name, w in weights.items():
                html_lines.append(f"<li><b>{name}:</b> {w*100:.1f}%</li>")
            html_lines.append("</ul>")
            
            html_lines.append(f"<p><i>Standard Matrix CR: {standard_cr:.4f}</i></p>")
            if standard_cr > 0.10:
                html_lines.append("<p style='color:red'><b>Warning:</b> Consistency Ratio > 0.10.</p>")
            
            self.ahp_preview_text.setHtml("".join(html_lines))
            self.current_ahp_weights = weights
            self.ahp_save_btn.setEnabled(True)
        else:
            weights, cr = calculate_ahp_weights(matrix, self.criteria_names)
            html_lines = ["<p><b>AHP Weights:</b></p><ul>"]
            for name, w in weights.items():
                html_lines.append(f"<li><b>{name}:</b> {w*100:.1f}%</li>")
            html_lines.append("</ul>")
            
            html_lines.append(f"<p><b>Consistency Ratio (CR):</b> {cr:.4f}</p>")
            if cr > 0.10:
                html_lines.append("<p style='color:red'><b>Warning:</b> Matrix is inconsistent (CR > 0.10). Consider revising your pairwise comparisons.</p>")
                
            self.ahp_preview_text.setHtml("".join(html_lines))
            self.current_ahp_weights = weights
            self.ahp_save_btn.setEnabled(True)

    def save_ahp_scheme(self) -> None:
        name = self.ahp_name_input.text().strip()
        if not name:
            QMessageBox.warning(self, "Validation Error", "Weight scheme name cannot be empty.")
            return

        try:
            self.weight_manager.save_scheme(self.entity_type, name, getattr(self, 'current_ahp_weights', {}))
            QMessageBox.information(self, "Success", f"AHP weight scheme '{name}' saved successfully.")
            self.ahp_name_input.clear()
            self.ahp_preview_text.clear()
            self.refresh_schemes_list()
